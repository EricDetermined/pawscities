...
export default function ForBusinessClient() {
  ...
  // Carry the referral code through to the claim page
  const claimHref = ref
    ? `/login?redirect=/business/claim?ref=${encodeURIComponent(ref)}`
    : '/login?redirect=/business/claim';
  const upgradeHref = ref
    ? `/login?redirect=/business/upgrade?ref=${encodeURIComponent(ref)}`
    : '/login?redirect=/business/upgrade';
