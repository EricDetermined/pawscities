              ...
              {fav.establishment ? (
                <div className="flex items-center gap-4 p-4">
                  <div className="w-16 h-16 rounded-lg bg-gray-100 shrink-0 overflow-hidden">
                    {fav.establishment.primary_image ? (
                      <img src={fav.establishment.primary_image} alt={fav.establishment.name} className="w-full h-full object-cover" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center text-2xl">📍</div>
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-gray-900 truncate">{fav.establishment.name}</h3>
                    <p className="text-sm text-gray-500 truncate">{fav.establishment.address}</p>
                    {fav.establishment.rating > 0 && (
                      <div className="flex items-center gap-1 mt-1">
                        <span className="text-yellow-500 text-xs">&#9733;</span>
                        <span className="text-xs text-gray-600">{fav.establishment.rating.toFixed(1)}</span>
                      </div>
                    )}
                  </div>
                </div>
