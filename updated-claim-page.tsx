  ...
  const handleSubmitNew = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setShowNewForm(false);
        setNewForm({
          name: '', address: '', city_id: '', category_id: '', description: '',
                ...
                {searchResults.map(est => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() => !nonClaimable && !est.isClaimed && handleSelectEstablishment(est)}
                    >
                      <div>
          ...
          <form onSubmit={handleSubmitClaim} className="space-y-4">
              ...
              <div>
                ...
                {emailDomainMatch === 'match' && (
                  <p className="mt-1 text-xs text-green-600 flex items-center gap-1">
                    <span>&#10003;</span> Email domain matches business website — faster verification
                  </p>
                )}
                {emailDomainMatch === 'mismatch' && selectedEstablishment?.website && (
                  <p className="mt-1 text-xs text-amber-600">
                    Tip: Using an email from your business domain speeds up verification
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="border-t border-gray-100 pt-4 mt-4">
              <h4 className="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2">
                <span>&#128274;</span> Ownership Verification
              </h4>
              <p className="text-xs text-gray-500 mb-3">
                To protect businesses from unauthorized claims, we verify ownership before approving your claim.
                Providing verification information speeds up the review process.
              </p>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Verification Method</label>
                <select
                  value={claimForm.verificationMethod}
                  onChange={(e) => setClaimForm(prev => ({ ...prev, verificationMethod: e.target.value }))}
                  className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none"
                >
              ...
              <div className="mt-3">
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Supporting Link or Reference (optional)
                </label>
                <input
                  type="url"
                  value={claimForm.verificationDoc}
                  onChange={(e) => setClaimForm(prev => ({ ...prev, verificationDoc: e.target.value }))}
                  className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none"
                  placeholder="e.g., Google Business URL, company website, or verification document link"
                />
                <p className="mt-1 text-xs text-gray-400">
                  Provide a link to your Google Business Profile, company website, or any supporting documentation.
                  Our team may contact you for additional verification if needed.
                </p>
              </div>
            </div>
      ...
      {showNewForm && (
        <div className="bg-white rounded-xl border p-6 mb-6">
          ...
          <p className="text-sm text-gray-500 mb-6">
            Submit your business to be listed on Paw Cities. We&apos;ll review your submission and get back to you
      ...
      {!selectedEstablishment && !showNewForm && (
        <div className="mt-6 bg-blue-50 rounded-xl border border-blue-100 p-6">
          ...
          <p className="text-sm text-blue-800 mb-3">
            We take business verification seriously to protect both business owners and our community.
            Here are the ways we verify ownership:
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-blue-800">
            <div className="flex items-start gap-2">
              <span className="text-blue-500 mt-0.5">&#10003;</span>
              <span><strong>Business domain email</strong> — Using an email matching your website domain is the fastest way to verify</span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-blue-500 mt-0.5">&#10003;</span>
              <span><strong>Business license</strong> — Upload or link to your business registration</span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-blue-500 mt-0.5">&#10003;</span>
              <span><strong>Google Business Profile</strong> — Link to your verified Google Business listing</span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-blue-500 mt-0.5">&#10003;</span>
              <span><strong>Phone verification</strong> — We may call the business number on file to confirm</span>
            </div>
          </div>
        </div>
      )}
